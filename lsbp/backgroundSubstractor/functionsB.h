...
__global__ void cuda_update_models(float* d_B_int, int* d_B_lsbp, float* d_D, float* d_R, float* d_T, float* d_int, int* d_lbp, bool* d_mask, int h, int w, int S, curandState_t* states){
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  int j = blockIdx.y * blockDim.y + threadIdx.y;
  if(i < h & j < w){
