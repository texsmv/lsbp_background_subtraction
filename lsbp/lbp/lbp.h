...
__device__ int lsbp_pixel(float* mat, int h, int w, int i, int j, float tau){
  float center_pixel = at2d(mat, i, j, w);
  int sum = 0;
  ...
  for (int k = 0; k < 8; k++)
  {
    ...
    vXYNeighbor[k] = at2d(mat, (int)x, (int)y, w);
  }
...
__device__ int olbp_pixel(float* mat, int h, int w, int i, int j, float tau){
  float center_pixel = at2d(mat, i, j, w);
  int sum = 0;
...
__device__ int xcslbp_pixel(float* mat, int h, int w, int xc, int yc, float tau){
  int xyNeighborPoints = 8;
  int fxRadius = 1;
  int fyRadius = 1;
  ...
  float centerVal = at2d(mat, xc, yc, w);
...
// lsbp kernel
__global__ void texture_kernel(float* d_mat, int* d_lbp, float tau, int h, int w, int texture_number){
  int row = blockIdx.x * blockDim.x + threadIdx.x;
  int col = blockIdx.y * blockDim.y + threadIdx.y;
  if(row > 0 && row < (h - 1) && col < (w - 1) && col > 0){
    if(texture_number == 0)
      at2d(d_lbp, row, col, w) = olbp_pixel(d_mat, h, w, row, col, tau);
    else if(texture_number == 1)
      at2d(d_lbp, row, col, w) = lsbp_pixel(d_mat, h, w, row, col, tau);
    else if(texture_number == 2)
      at2d(d_lbp, row, col, w) = xcslbp_pixel(d_mat, h, w, row, col, tau);
  }
...
void cuda_texture(float* d_mat, int* d_lbp, int h, int w, float tau, dim3 block, dim3 grid, int texture_number){
  texture_kernel<<<grid, block>>>(d_mat, d_lbp, tau, h, w, texture_number);
}
